...
def main():
    ...
    if git_exists:
        ...
        try:
            ...
            if DEBUG_MODE:
                log_debug(f"Git fetch result: {fetch.returncode}", debug_log)
                if fetch.stdout:
                    log_debug(f"Git fetch stdout: {fetch.stdout}", debug_log)
                if fetch.stderr:
            ...
            if fetch.returncode != 0:
                ...
                if DEBUG_MODE:
                    log_debug(f"Git reset result: {reset.returncode}", debug_log)
                    if reset.stdout:
                        log_debug(f"Git reset stdout: {reset.stdout}", debug_log)
                    if reset.stderr:
                        log_debug(f"Git reset stderr: {reset.stderr}", debug_log)
                if reset.returncode == 0:
                    print_status("Up to date with origin/main", "ok")
                else:
                    print_status("Could not reset to origin/main (this is OK)", "warning")
        except Exception as e:
